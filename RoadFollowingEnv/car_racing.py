    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            if not tile.road_visited:
                ...
                self.env.state.num_tiles_visited += 1
...
class RoadFollowingEnv(gym.Env, EzPickle):
    ...
    def _step(self, action):
        ...
        if action is not None:
            self.reward -= 0.1
            if self.state.num_tiles_visited == len(self.track):
                done = True
    ...
    def render(self, mode="human"):
        if self.viewer is None:
            ...
            if self.title is not None:
                self.viewer.window.set_caption(self.title)
            self.score_label = pyglet.text.Label('0000', font_size=36,
                x=20, y=WINDOW_H*2.5/40.00, anchor_x='left', anchor_y='center',
                color=(255,255,255,255))
            self.value_label = pyglet.text.Label('', font_size=16,
                x=WINDOW_W-20, y=WINDOW_H*3.0/40.00, anchor_x='right', anchor_y='center',
                color=(255,255,255,255))
            self.distance_traveled_label = pyglet.text.Label('0000', font_size=16,
                x=WINDOW_W-20, y=WINDOW_H*2.0/40.00, anchor_x='right', anchor_y='center',
                color=(255,255,255,255))
        ...
        if mode=="rgb_array": # agent can call or not call env.render() itself when recording video.
            win.flip()
